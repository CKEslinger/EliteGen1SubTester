...
Private m_szCBReceive As String
Private m_szATCommand As String
Private m_szCBCommand As String
...
Public Property Let PSOutput(bTurnPowerOn As Boolean)
    ...
    If bTurnPowerOn Then
        ' Set Inrush current limit
        If True = PowerSupply1_Amp_Limit("INRUSH") Then
    ...
    Select Case g_fMainTester.ComPortToPowerSupply.CommEvent
        Case comEvSend
            ...
            Select Case g_ePS_State
                ...
                Case ePS_SET_INRUSH_CURRENT_LIMIT
                    g_ePS_COM_Status = eSTAT_SUCCESS
    
                Case ePS_SET_NOMINAL_CURRENT_LIMIT
                    g_ePS_COM_Status = eSTAT_SUCCESS
...
UglyJump:
                ...
                Else
                    ...
                    If &HD = cChar Or &HA = cChar Then
                        If "" <> m_szUnsolicitated Then
                            LogEliteComPortEventsToFile (m_szUnsolicitated)
                            If False = m_bDevPOST_OK And "" <> m_szDevPOST_OK_Message Then
                                If InStr(m_szUnsolicitated, m_szDevPOST_OK_Message) > 0 Then
                                    ' Device POST message found in unsolicitated data stream
                                    m_szDevPOST_Message = m_szUnsolicitated
                                    m_bDevPOST_OK = True
                                    ' Now look for: "INFO        Done initializing modem. "
                                    ' This on the other hand would suck: "ERR   Modem unregistered too long! Reset modem to try from scratch."
                                End If
                            End If
                            ' m_inputStream = ""
                            m_szUnsolicitated = ""
    ...
    Else
        Select Case g_eElite_State
            ...
            Case eElite_GET_IMEI, eElite_GET_IMSI, eElite_GET_CCID, eElite_GET_VOLTS, eElite_SET_SERIAL_NUM, eElite_GET_SERIAL_NUM, eElite_LOCK_GPS, _
                 ...
                 eElite_SET_SMS_ADDRESS, eElite_SET_AUTO_RESET_MIN, eElite_GET_AUTO_RESET_MIN, eElite_SET_SMS_MODE, eElite_GET_SMS_MODE, eElite_ANTITHEFT, _
                 eElite_CHECK_IGN, eElite_CHECK_STARTER, eElite_GET_SIGNAL_STRENGTH, eElite_GET_GPS_LOCATION, _
                 eElite_GET_SCA, eElite_GET_VOLTS_DUT
            ...
            Case eElite_SET_STATION_ID_FORMAT, eElite_ABORT_SMS_SS_PLMN_SELECT
                If m_iReceivedIndex < 1 Then
                    g_fMainTester.LogMessage ("g_eElite_State: Index=" & m_iReceivedIndex & " (supposed to >=1)")
                    g_fMainTester.LogMessage ("g_eElite_State: (-0)=" & m_aszReceivedLines(m_iReceivedIndex))
                    g_eElite_Status = eSTAT_ERROR
            ...
            Case eElite_DISABLE_AUTO_GSM_REG, eElite_SET_SCA, eElite_GET_GSM_REG_STATUS, eElite_SET_GSM_MODEM_FLOW_CTRL, eElite_SET_GSM_MODEM_BAUD_RATE, eElite_CLR_GSM_MODEM_BAUD_RATE, eElite_RESET_GSM_MODULE, eElite_GET_GSM_COPS
                If m_iReceivedIndex < 1 Then
                    g_fMainTester.LogMessage ("g_eElite_State: Index=" & m_iReceivedIndex & " (supposed to >=1)")
                    g_fMainTester.LogMessage ("g_eElite_State: Comd=" & m_szATCommand)
                    g_fMainTester.LogMessage ("g_eElite_State: Parm=" & m_szParameter & "," & m_iParameter)
            ...
            Case eElite_BGS2_MODEM_ID
                If m_szStateResponses(eElite_BGS2_MODEM_ID) = m_aszReceivedLines(m_iReceivedIndex) Then
                    g_eElite_Status = eSTAT_SUCCESS
                    m_szParameter = m_aszReceivedLines(m_iReceivedIndex - 1)
                End If
            
            Case eElite_BGS2_MODEM_SVN
                If m_szStateResponses(eElite_BGS2_MODEM_SVN) = m_aszReceivedLines(m_iReceivedIndex) Then
                    g_eElite_Status = eSTAT_SUCCESS
                    m_szParameter = m_aszReceivedLines(m_iReceivedIndex - 1)
                End If
            
            Case eElite_BGS2_SHUTOFF
                If m_szStateResponses(eElite_BGS2_SHUTOFF) = m_aszReceivedLines(m_iReceivedIndex) Then
                    g_eElite_Status = eSTAT_SUCCESS
                End If
            ...
            Case etELITE_STATES.eElite_ENABLE_BYPASS, etELITE_STATES.eElite_DISABLE_BYPASS
...
Public Sub ComPortToCallBoxEventProcessor()
    ...
    Select Case g_fMainTester.ComPortToCallBox.CommEvent
        Case comEvSend
            cCR = 0
            cLF = 0
            Select Case g_eCB_State
                Case eCB_VERIFY, eCB_CHECK_FOR_ERROR, eCB_GET_ERROR
                    '   Nothing to do here but wait for the response.

                Case etCB_STATES.eCB_CLEAR_ERROR
                    g_eCB_COM_Status = etCOM_STATUS.eSTAT_ERROR
                
                Case etCB_STATES.eCB_RESET, etCB_STATES.eCB_LOC_UPD, etCB_STATES.eCB_ESC, _
                etCB_STATES.eCB_END_REMOTE, eCB_CONFIG_GSM, eCB_CONFIG_BCCH, eCB_CONFIG_PCH, _
                eCB_CONFIG_PWR_LVL_BCCH, eCB_CONFIG_PWR_LVL_TCH, eCB_CONFIG_PWR_LVL_RF, _
                eCB_CONFIG_PRE_ATTEN

                    g_eCB_COM_Status = etCOM_STATUS.eSTAT_SUCCESS

                Case Else
                    MsgBox "Invalid state after message sent to CB: (" & g_eCB_State & ")"
                    g_eCB_COM_Status = etCOM_STATUS.eSTAT_ERROR
        ...
        Case comEvReceive
            '   Go get the character that has just come in from the Power Supply
            '   and add it to the receive buffer.
            While g_fMainTester.ComPortToCallBox.InBufferCount > 0
                cCR = cLF
                cLF = Asc(g_fMainTester.ComPortToCallBox.Input)
                If cCR = &HD And cLF = &HA Then
                    '   A complete response has been received from the CB.  Go Step
                    '   to the next CB state.
                    Select Case g_eCB_State
                        Case etCB_STATES.eCB_NULL
                            '       Just ignore whatever comes back

                        Case etCB_STATES.eCB_VERIFY
                            ' g_eCB_State = etCB_STATES.eCB_NULL
                            Dim strPtr As Integer
                            Dim strToFind As String
                            Dim strToFindLen As Integer
                            strToFind = "Willtek"
                            strPtr = InStr(m_szCBReceive, ",")
                            strToFindLen = Len(strToFind)
                            If strToFindLen < strPtr And strToFind = Mid(m_szCBReceive, strPtr - strToFindLen, strPtr - 1) Then
                                g_eCB_COM_Status = etCOM_STATUS.eSTAT_SUCCESS
                            Else
                                g_eCB_COM_Status = etCOM_STATUS.eSTAT_ERROR
                            End If

                        Case etCB_STATES.eCB_CHECK_FOR_ERROR
                            If m_szCBReceive = "" Or "+0" Then
                                g_eCB_COM_Status = etCOM_STATUS.eSTAT_SUCCESS
                            Else
                                TransmitToCB etCB_STATES.eCB_GET_ERROR
                            End If

                        Case etCB_STATES.eCB_GET_ERROR
                            MsgBox "Call Box Error: " & m_szCBReceive
                            TransmitToCB etCB_STATES.eCB_CLEAR_ERROR

                        Case eCB_CLEAR_ERROR
                            '   Just ignore whatever comes back

                        Case eCB_CONFIG_GSM, eCB_CONFIG_BCCH, eCB_CONFIG_PCH, eCB_CONFIG_PWR_LVL_BCCH, _
                        eCB_CONFIG_PWR_LVL_TCH, eCB_CONFIG_PWR_LVL_RF, eCB_CONFIG_PRE_ATTEN
                            ' No way response expected
                            g_eCB_COM_Status = etCOM_STATUS.eSTAT_SUCCESS

                        Case Else
                            MsgBox "An illegal state (" & g_eCB_State & ") in the CB Com state machine has been reached" & vbCrLf & vbCrLf & _
                                   "  -Received: '" & m_szCBReceive & "'"
                            g_eCB_COM_Status = etCOM_STATUS.eSTAT_ERROR

                    End Select
                    If g_eCB_State <> eCB_NULL Then
                    End If
                ElseIf cLF <> &HD And cLF <> &HA Then
                    '   Add on whatever was received to the CB receive string.
                    m_szCBReceive = m_szCBReceive + Chr(cLF)
                End If
            Wend
    ...
    End Select
    If g_eCB_COM_Status = etCOM_STATUS.eSTAT_RUNNING Then
        '   OK, what ever has happened, things haven't finished yet so start
        '   the timeout going again.
        g_fMainTester.tmrCBCom.Enabled = True
    End If
...
Public Sub Destroy()
    ...
    g_eCB_COM_Status = eSTAT_SHUTTING_DOWN
    m_bKillTimers = True
    g_fMainTester.tmrPSCom.Enabled = False
    ...
    g_fMainTester.tmrCBCom.Enabled = False
    g_fMainTester.tmrCBCom.Interval = 0
...
Public Sub TransmitToPS(eState As etPS_STATES)
    ...
    If g_fMainTester.ComPortToPowerSupply.PortOpen = True Then
        Select Case g_ePS_State
            ...
            Case ePS_SET_INRUSH_CURRENT_LIMIT
                If Nothing Is g_oSelectedModel Then
                    dMaxCurrent = 1.3
                Else
                    dMaxCurrent = CDbl(g_oSelectedModel.selectSingleNode("Power").selectSingleNode("Inrush_Current").Attributes.getNamedItem("Max").Text)
                End If
                dMaxCurrent = dMaxCurrent + 0.1
                g_fMainTester.ComPortToPowerSupply.Output = "CURR " & dMaxCurrent & vbCrLf

            Case ePS_SET_NOMINAL_CURRENT_LIMIT
                dMaxCurrent = CDbl(g_oSelectedModel.selectSingleNode("Power").selectSingleNode("Nominal_Current").Attributes.getNamedItem("Max").Text)
                dMaxCurrent = dMaxCurrent + 0.1
                g_fMainTester.ComPortToPowerSupply.Output = "CURR " & dMaxCurrent & vbCrLf
    ...
    If True = g_fMainTester.ComPortToElite.PortOpen Then
        Select Case g_eElite_State
            ...
            Case eElite_ENABLE_BYPASS
                ...
                ' Calls MSCOMM directly and no response expected
            ...
            Case eElite_BGS2_MODEM_ID
                If "" = m_szParameter Then
                    m_szATCommand = "AT+CGSN"
                Else
                    m_szATCommand = "AT^SGSN" & m_szParameter
                End If

            Case eElite_BGS2_MODEM_SVN
                m_szATCommand = "AT^SSVN" & m_szParameter

            Case eElite_BGS2_SHUTOFF
                m_szATCommand = "AT^SMSO"
...
'                m_szATCommand = "AT^SCID"
            ...
            Case eElite_CHECK_BUZZER
                ...
                ' If the buzzer output is verified through hardware
                    g_fMainTester.ComPortToElite.Output = m_szATCommand & vbCr
                    g_fMainTester.LogMessage ("Transmitted 'Mt4' to device. Response not verifed")
                    g_eElite_State = eElite_NULL
                End If

            Case eElite_SET_MANUAL_REG_MODE_AND_SELECT_OPERATOR
                m_szATCommand = "AT+COPS=1,2," & oAttributes.getNamedItem("Station").Text

            Case eElite_SET_STATION_ID_FORMAT
                m_szATCommand = "AT+COPS=3,2"

            Case eElite_GET_GSM_COPS
                m_szATCommand = "AT+COPS?"
            ...
            Case eElite_GET_AUTO_RESET_MIN
                m_szATCommand = "AT$RESETMIN?"

            Case eElite_SET_AUTO_RESET_MIN
                m_szATCommand = "AT$RESETMIN=" & m_szParameter

            Case eElite_GET_SCA
                m_szATCommand = "AT+CSCA?"
                
            Case eElite_SET_SCA
                m_szATCommand = "AT+CSCA=" & m_szParameter
            ...
            Case eElite_DISABLE_AUTO_GSM_REG
                m_szATCommand = "AT+COPS=2"
            ...
            ' WTF? Commented out on 6/7/2013
'            Case 1000
'                m_szATCommand = "AT+CREG?"
            
            ' JMB modified 6-7-13
            Case eElite_GET_VOLTS_DUT
                m_szATCommand = "M9"
        ...
        If g_eElite_State <> eElite_NULL Then
            ...
            g_fMainTester.ComPortToElite.Output = m_szATCommand & vbCr
        Else
            '   Stop the timer, nothing got sent out or no response expected
            g_fMainTester.tmrEliteCom.Enabled = False
            g_eElite_Status = eSTAT_SUCCESS
        End If
    Else
...
'   This function transmits a string to the Call Box and starts the CB COM
'   state machine going to handle all of the events that will during this COM
'   session.
Public Sub TransmitToCB(eState As etCB_STATES)

    g_eCB_COM_Status = eSTAT_RUNNING
    g_eCB_State = eState
    m_szPSReceive = ""
    '   Start the timer going
    g_fMainTester.tmrCBCom.Enabled = True
    If g_fMainTester.ComPortToCallBox.PortOpen = True Then
        Select Case g_eCB_State
            Case etCB_STATES.eCB_NULL
                '   Ignore this state
            ...
            Case etCB_STATES.eCB_GET_ERROR
                m_szCBCommand = "SYST:ERR?"

            Case etCB_STATES.eCB_CLEAR_ERROR
                m_szCBCommand = "*CLS"

            Case etCB_STATES.eCB_RESET
                m_szCBCommand = "*RST"

            Case etCB_STATES.eCB_LOC_UPD
                m_szCBCommand = "CALL:LUPD"

            Case etCB_STATES.eCB_ESC
                ' According to Willtek manual this won't work unless "software
                ' handshake protocol (is) deactivated on the controller"
                ' May want to use the "Delay Escape" command "SYST:UBR:DEL x"
                ' where x is the delay in seconds before the ESC command is
                ' executed on the callbox
                m_szCBCommand = "SYST:UBR"
            
            Case etCB_STATES.eCB_END_REMOTE
                m_szCBCommand = "SYST:COMM:LOC"

            Case etCB_STATES.eCB_CONFIG_GSM
                m_szCBCommand = "CONF:SYST GSM1900"
            
            Case etCB_STATES.eCB_CONFIG_BCCH
                m_szCBCommand = "CONF:GSM1900:BS:CCH:ARFC 661"
            
            Case etCB_STATES.eCB_CONFIG_PCH
                m_szCBCommand = "CONF:GSM1900:BS:TCH:ARFC 660"
            
            Case etCB_STATES.eCB_CONFIG_PWR_LVL_BCCH
                m_szCBCommand = "CONF:GSM1900:MS:CCH:PLEV 30"
            
            Case etCB_STATES.eCB_CONFIG_PWR_LVL_TCH
                m_szCBCommand = "CONF:GSM1900:MS:TCH:PLEV 30"
            
            Case etCB_STATES.eCB_CONFIG_PWR_LVL_RF
                m_szCBCommand = "RFG:GSM1900:LEV -48"
            
            Case etCB_STATES.eCB_CONFIG_PRE_ATTEN
                m_szCBCommand = "CONF:GSM1900:PATT:LEV 0,0"
            
            Case Else
                MsgBox "CB transmit, illegal state (" & g_eCB_State & ")"
                g_eCB_State = eCB_NULL
        End Select
        
        g_fMainTester.ComPortToCallBox.Output = m_szCBCommand & vbCrLf
        If eCB_NULL <> g_eCB_State Then
            g_fMainTester.LogMessage ("Transmitted '" & m_szCBCommand & "' to call box")
        End If
    Else
    ...
    If g_eCB_State = eCB_NULL Then
        '   Stop the timer, nothing got sent out
        g_fMainTester.tmrCBCom.Enabled = False
        g_eCB_COM_Status = eSTAT_SUCCESS
    End If
...
Public Function PowerSupply1_Current() As Double
    PowerSupply1_Current = 0
    TransmitToPS ePS_MEAS_CURRENT
    While g_ePS_COM_Status = eSTAT_RUNNING
        DoEvents
    Wend
    If g_ePS_COM_Status = eSTAT_SUCCESS Then
        If False = IsNumeric(m_szPSReceive) Then
            ' g_ePS_COM_Status = eSTAT_ERROR
            g_fMainTester.LogMessage ("Power supply return value is not numeric")
...
'               "Status = " & g_ePS_COM_Status & ", received = '" & m_szPSReceive, vbCritical
        PowerSupply1_Current = -9999
    End If
    If -9999 = PowerSupply1_Current Then
        g_fMainTester.LogMessage ("Error Receiving Measured Current From Power Supply" & vbCrLf & vbCrLf & "Status = " & g_ePS_COM_Status & ", received = '" & m_szPSReceive)
        TransmitToPS ePS_CLEAR_ERROR
        While g_ePS_COM_Status = eSTAT_RUNNING
            DoEvents
        Wend
...
Public Function PowerSupply1_Amp_Limit(szLimit As String) As Boolean

    If szLimit = "INRUSH" Then
        TransmitToPS ePS_SET_INRUSH_CURRENT_LIMIT
    Else
        TransmitToPS ePS_SET_NOMINAL_CURRENT_LIMIT
    End If
    ...
    Wend
    If g_ePS_COM_Status = eSTAT_SUCCESS Then
        PowerSupply1_Amp_Limit = True
    Else
        MsgBox "Error Setting Power Supply Current Limit" & vbCrLf & vbCrLf & _
               "Status = " & g_ePS_COM_Status, vbCritical
        PowerSupply1_Amp_Limit = False
        TransmitToPS ePS_CLEAR_ERROR
        While g_ePS_COM_Status = eSTAT_RUNNING
            DoEvents
        Wend
...
Private Function CheckForCBError() As Boolean

    CheckForCBError = False
    TransmitToCB eCB_CHECK_FOR_ERROR
    While g_eCB_COM_Status = eSTAT_RUNNING
        DoEvents
    Wend
    If g_eCB_COM_Status = eSTAT_SUCCESS Or g_eCB_COM_Status = eSTAT_TIMEOUT Then
        '   If the Power Supply doesn't respond then it doesn't have any error
        '   messages for us (so this is a good thing).
        g_eCB_COM_Status = eSTAT_SUCCESS
        CheckForCBError = True
    Else
        '   Query the Power Supply for whatever error it has detected
        TransmitToCB eCB_GET_ERROR
        While g_eCB_COM_Status = eSTAT_RUNNING
            DoEvents
        Wend
    End If
...
Public Function VerifyOpenAT() As Boolean
    ...
    If Not VerifyOpenAT Then
        ...
        m_szParameter = ""
        VerifyOpenAT = SendATCommand(eElite_verify, 3000)
        If Not VerifyOpenAT Then
            g_fMainTester.LogMessage " Response: " & m_szParameter
            m_szParameter = ""
            VerifyOpenAT = SendATCommand(eElite_verify, 3000)
...
Public Function SetServerIP(iServer As Integer, szIP As String) As Boolean
    ...
    m_szParameter = Chr(&H22) & CStr(iServer) & Chr(&H22) & "," & Chr(&H22) & szIP & Chr(&H22)
...
Public Function GetVoltsDUT(szmV As String) As Boolean
    ' Read voltage from DUT
    GetVoltsDUT = SendATCommand(eElite_GET_VOLTS_DUT)
    If GetVoltsDUT Then
        szmV = m_szParameter
    End If
...
Public Function CheckLED(iLED As Integer) As Boolean
    ...
    Select Case iLED
    Case 0
        ' TODO: Root cause why elite grn LED turn on gets stuck waiting for response
        ' Theory: The "M7" command doesn't return a value. It simply returns "OK".
        '         The parser is probably expecting the "OK" on the wrong response line.
...
Public Function GetOperatorStatus(Optional szOpStat As String) As Boolean

    '   Get the modem's operator/network registration status and mode
    GetOperatorStatus = SendATCommand(eElite_GET_GSM_COPS, 5000)
    
    If GetOperatorStatus Then
        szOpStat = m_szParameter
    End If
    startup.Delay 1#
...
Public Function VerifySignalStrengthGPRS(szMin As String, szMax As String, Optional bMainTester As Boolean = True) As Boolean
    Dim iSigStrengthRaw As Integer
    Dim iSigStrengthPlusAtten As Integer
    ...
    If bAtCmdStatus Then
        ...
        If InStr(1, m_szParameter, "+CSQ: ") = 1 Then
            ...
            iSigStrengthRaw = CInt(Val(Right(m_szParameter, Len(m_szParameter) - Len("+CSQ: "))))
            iSigStrengthPlusAtten = iSigStrengthRaw + g_iGSM_Attenuation
            
            ' Max csq is 31. csq of 99 is no signal
            If iSigStrengthPlusAtten > 30 And iSigStrengthPlusAtten < (32 + g_iGSM_Attenuation) Then
                iSigStrengthPlusAtten = 31
            ElseIf iSigStrengthPlusAtten > (31 + g_iGSM_Attenuation) Then
                iSigStrengthPlusAtten = 99
            End If
            
            If iSigStrengthPlusAtten < CInt(Val(szMin)) Or iSigStrengthPlusAtten > CInt(Val(szMax)) Then
                g_fMainTester.LogMessage "Signal strength is bad, got " & iSigStrengthPlusAtten & ", range = (" & szMin & "," & szMax & ")"
            Else
                ' GPRS signal strength is in expected range
                szMax = CStr(iSigStrengthPlusAtten)
            End If
            szMin = szMax
        Else
...
Public Function FindSimulator(Optional bSendLocUpd As Boolean = False) As Boolean
    Dim bAtCmdStatus As Boolean
    Dim szMin As String
    Dim szMax As String
    ...
    ' Analyze device registration state with AT+CREG?
    m_szParameter = ""
    If False = SendATCommand(eElite_GET_GSM_REG_STATUS) Then
       'Try again
        g_fMainTester.LogMessage (" No response from reg. status command (" & m_szParameter & "). Retrying")
        startup.Delay 1#
    ...
    If bAtCmdStatus Then
        ...
        If Not InStr(1, m_szParameter, "+CREG: 2,5,") = 1 Then
            ' Could indicate that there is a problem with the cell site simulator box
            g_fMainTester.LogMessage (" Registration invalid (" & m_szParameter & ").")
        
        Else
            g_fMainTester.LogMessage (" Registration valid (" & m_szParameter & ").")
            m_szParameter = ""
            g_fMainTester.UpdateTestCycleTime
            ' DoEvents
            SendATCommand eElite_GET_GSM_COPS, 5000
            If Not InStr(13, m_szParameter, "00101") = 13 Then
                ' Not registered yet
                g_fMainTester.LogMessage (" Not registered to call box (" & m_szParameter & ").")
            Else
                ' We are registered with the cell site simulator and are registered in roaming mode
                g_fMainTester.LogMessage (" Registered with call box (" & m_szParameter & ").")
                FindSimulator = True
            End If
        End If
    End If
...
Public Function SetModeAndOperator(szMode As String, Optional szPLMN As String = "00000", Optional bMainTester As Boolean = True) As Byte
    ...
    Do
        ...
        Case eCME_ERR_527
            ...
            Else
                startup.Delay CDbl(CME_ERR_527_DELAY)
            End If
        Case eCME_ERR_529
            ' Somebody forgot to push the "F2" (LOC UPD) button on the simulator
            Exit Do
...
Public Function GetSCAString() As String
    Dim iStrIdx As Integer
    GetSCAString = ""
    
    If g_eElite_State = eElite_NULL Then
        '   send AT command to get the Service Center Address
        If SendATCommand(eElite_GET_SCA) Then
            ' Parse the return string
            'iStrIdx = InStr(1, m_szParameter, ":")
            'm_szParameter = Trim(Right(m_szParameter, Len(m_szParameter) - iStrIdx))
            GetSCAString = m_szParameter
        End If
    End If
...
End Function
Public Function SendSCAString(strSCA As String) As Boolean

    SendSCAString = False
    m_szParameter = strSCA
    If g_eElite_State = eElite_NULL Then
        '   send AT command to set the Service Center Address
        SendSCAString = SendATCommand(eElite_SET_SCA)
    End If
...
Public Function GetSMSReplyAddress(szSMSReplyAddr As String, Optional iAddrIdx As Integer = 0) As Boolean
    ...
    End If
End Function
Public Function GetResetMin(szResetMin As String) As Boolean
    GetResetMin = False
    GetResetMin = SendATCommand(eElite_GET_AUTO_RESET_MIN)
    If GetResetMin Then
        szResetMin = m_szParameter
    End If
End Function
Public Function SetResetMin(strResetMin As String, Optional szResetIdx As String = "1") As Boolean

    SetResetMin = False
    m_szParameter = strResetMin & "," & szResetIdx
    If g_eElite_State = eElite_NULL Then
        '   send AT command to set the SMS reset minimum
        SetResetMin = SendATCommand(eElite_SET_AUTO_RESET_MIN)
    End If
...
Public Function SendATCommand(eATCommand As etELITE_STATES, Optional lSerialComsTimeout As Long = SERIAL_COMS_TIMEOUT) As Boolean
    ...
    Dim dFailSafeTimeOut
    Dim iFailSafeTimeOut As Integer
    Dim bFailSafeRetry As Boolean
    
    SendATCommand = False
    iFailSafeTimeOut = 100
    bFailSafeRetry = True
    ...
    Do
        ...
        While g_eElite_Status = eSTAT_RUNNING
            ...
            If Not g_fIO.IsTesterClosed Then
                g_eElite_Status = eSTAT_ERROR
            ElseIf True = g_fMainTester.tmrEliteCom.Enabled And iFailSafeTimeOut = 0 Then
                ' Shouldn't ever get here, but we do sometimes
                If True = bFailSafeRetry Then
                    Debug.Print "Cmd " & eATCommand & " failed; Forced time out. Retrying..."
                    g_fMainTester.LogMessage "Forced serial comm. time out. Retrying..."
                    ' Turn off fail safe retrying
                    bFailSafeRetry = False
                    
                    ' Reset timer. If expires again set fail status and drain out of function
                    iFailSafeTimeOut = 100
                    Exit Do
                End If
                g_fMainTester.LogMessage "Forced serial comm. time out"
                g_eElite_Status = eSTAT_TIMEOUT
            End If
            iFailSafeTimeOut = iFailSafeTimeOut - 1
            startup.Delay 0.1
        Wend
    ...
    Else
        ...
        If eSTAT_ERROR = g_eElite_Status Then
            szMessage = szMessage + "Unrecognized response or unknown error"
        ElseIf eSTAT_TIMEOUT = g_eElite_Status Then
...
Public Function OpenEliteComPort(bOpenElite As Boolean, Optional bWarnOnError As Boolean = False) As Boolean
    ...
    If True = bOpenElite Then
        If 0 = m_fhNumEliteComPortLogFile Then
            ...
            g_fMainTester.LogMessage ("Opened COM Port log file")
        Else
        ...
        If False = g_fMainTester.ComPortToElite.PortOpen Then
            ...
            If False = g_fMainTester.ComPortToElite.PortOpen Then
                ' Occasionally doesn't open so wait 2 s and try again
                g_fMainTester.LogMessage ("Device COM Port failed to open. Retrying...")
                startup.Delay 2#
                g_fMainTester.ComPortToElite.PortOpen = True
            End If
        Else
            g_fMainTester.LogMessage ("Device COM Port already open. Closing COM port")
            g_fMainTester.ComPortToElite.PortOpen = False
            startup.Delay 1#
            g_fMainTester.LogMessage ("Opening COM port...")
            g_fMainTester.ComPortToElite.PortOpen = True
        End If
    ...
    Else
        ...
        If Not 0 = m_fhNumEliteComPortLogFile Then
            ...
            g_fMainTester.LogMessage ("Closing COM Port log file...")
...
Public Function ToggleEliteComPortLogFileCloseThenOpen() As Boolean
    If 0 <> m_fhNumEliteComPortLogFile Then
    '    g_fMainTester.LogMessage ("Closing COM Port log file...")
        Close #m_fhNumEliteComPortLogFile
        m_fhNumEliteComPortLogFile = 0
    ...
    If 0 = m_fhNumEliteComPortLogFile Then
        ToggleEliteComPortLogFileCloseThenOpen = False
    Else
        ToggleEliteComPortLogFileCloseThenOpen = True
    End If
End Function
Public Function WaitForPost(iWaitForPOST As Integer) As Boolean
    ...
    If False = g_fMainTester.ComPortToElite.PortOpen Then
        ...
        End If
    Else
        g_fMainTester.LogMessage ("COM port is open")
        If g_eElite_State <> eElite_NULL Then
            ' That's a problem
        End If
    End If
    ...
    Else
        m_bDevPOST_OK = False
        iWaitForPOST = 30
        m_szDevPOST_OK_Message = m_szStateResponses(eElite_INIT_COMPLETE)
        While False = m_bDevPOST_OK And 0 <> iWaitForPOST
            iWaitForPOST = iWaitForPOST - 1
            startup.Delay 1#
        Wend
        ...
        If False = m_bDevPOST_OK Then
            ...
            WaitForPost = False
        Else
            g_fMainTester.LogMessage ("Received message '" & m_szDevPOST_OK_Message & "' from device")
            WaitForPost = True
        End If
    End If
End Function

Private Function ReceiveUnsollicitedMessage(eState As etELITE_STATES, Optional lSerialComsTimeout As Long = SERIAL_COMS_TIMEOUT) As Boolean
    Dim szMessage As String
...
Public Function BGS2ModemID(szModemID As String, Optional etModemType As etELITE_STATES = eElite_BGS2_MODEM_ID) As Boolean
    BGS2ModemID = False
    If "" = szModemID Or "?" = szModemID Then
        ' Read the modem ID
        m_szParameter = szModemID
        BGS2ModemID = SendATCommand(etModemType)
        If BGS2ModemID Then
            szModemID = m_szParameter
        End If
    Else
        ...
        m_szParameter = "=" & szModemID
        BGS2ModemID = SendATCommand(etModemType)
        If True = BGS2ModemID Then
            If etELITE_STATES.eElite_BGS2_MODEM_ID = etModemType Then
                ' The write command returns these result codes:
                ' 0 Customer IMEI successfully written'
                ' 1 Invalid IMEI
                ' 2 Writing IMEI failed
                ' 3 Freezing IMEI failed
                ' 4 Customer IMEI already written
                If "^SGSN: 0" <> Left(m_szParameter, 8) Then
                    BGS2ModemID = False
                End If
            End If
        End If
...
Public Function BGS2ModemSVN(szModemSVN As String, Optional etModemType As etELITE_STATES = eElite_BGS2_MODEM_ID) As Boolean
    BGS2ModemSVN = False
    If "" = szModemSVN Or "?" = szModemSVN Then
        ' Read the modem Software Version Number
        m_szParameter = "?"
        BGS2ModemSVN = SendATCommand(eElite_BGS2_MODEM_SVN)
        If BGS2ModemSVN Then
            szModemSVN = m_szParameter
        End If
    Else
        ' Write the modem Software Version Number
        m_szParameter = "=" & szModemSVN
        BGS2ModemSVN = SendATCommand(eElite_BGS2_MODEM_SVN)
    End If
